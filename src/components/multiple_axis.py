...
class MultipleAxisVisual(CompoundVisual):
    def __init__(self, pos: np.ndarray, color: str = "black", labels=None):
        ...
        self._lines = LineVisual(
            pos=self.pos, method="gl", color=color, connect="segments", antialias=True
        )
        self._text = TextVisual(
            text=labels,
            color=color,
            bold=True,
            italic=True,
    ...
    def set_data(self, pos: np.ndarray) -> None:
        for i in range(pos.shape[0]):
            self.pos[2 * i + 1] = pos[i]
